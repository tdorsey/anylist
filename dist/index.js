...
var AnyList = class extends import_events.EventEmitter {
  constructor(options) {
    ...
    this.authClient = import_got.default.extend({
      headers: {
  ...
  async _loadCredentials() {
    ...
    if (!import_fs.default.existsSync(this.credentialsFile)) {
      console.info("Credentials file does not exist, not loading saved credentials");
      return;
    }
    try {
      const encrypted = await import_fs.default.promises.readFile(this.credentialsFile);
  ...
  async _storeCredentials() {
    ...
    try {
      ...
      await import_fs.default.promises.writeFile(this.credentialsFile, encrypted);
    } catch (error) {
  ...
  _encryptCredentials(credentials, secret) {
    ...
    const key = import_crypto.default.createHash("sha256").update(String(secret)).digest("base64").slice(0, 32);
    const iv = import_crypto.default.randomBytes(16);
    const cipher = import_crypto.default.createCipheriv("aes-256-cbc", import_buffer.Buffer.from(key), iv);
    let encrypted = cipher.update(plain);
    encrypted = import_buffer.Buffer.concat([encrypted, cipher.final()]);
  ...
  _decryptCredentials(credentials, secret) {
    ...
    const key = import_crypto.default.createHash("sha256").update(String(secret)).digest("base64").slice(0, 32);
    const decipher = import_crypto.default.createDecipheriv("aes-256-cbc", import_buffer.Buffer.from(key), import_buffer.Buffer.from(encrypted.iv, "hex"));
    let plain = decipher.update(import_buffer.Buffer.from(encrypted.cipher, "hex"));
    plain = import_buffer.Buffer.concat([plain, decipher.final()]);
  ...
  _setupWebSocket() {
    ...
    this.ws = new import_reconnecting_websocket.default("wss://www.anylist.com/data/add-user-listener", [], {
