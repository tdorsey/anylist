...
// Pydantic-like schema for List validation
const ListSchema = Joi.object({
	identifier: Joi.string().required(),
	listId: Joi.string().allow(null, ''), // ParentId
...
class List {
   ...
   */
	constructor(list, {client, protobuf, uid}) {
		...
		const {error, value} = ListSchema.validate(list || {});

		if (error) {
			const errorMessage = error.details.map(detail => detail.message).join(', ');
			throw new Error(`List validation failed: ${errorMessage}`);
		}

		this.identifier = value.identifier;
		this.parentId = value.listId;
		this.name = value.name;

		this.items = value.items.map(i => new Item(i, {client, protobuf, uid}));
		this.client = client;
